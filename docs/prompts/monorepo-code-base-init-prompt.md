...
3) Implement backlog items from `docs/features/enablers-and-spikes.md` only:
...
4) Apply backend tech constraints from `backend/docs/initial-requirements/04-tech-requirements.md` (Node.js, Express or Express/Fastify per variant, PostgreSQL, query-layer choice with rationale, JWT middleware, `zod`/`joi` validation, and migrations).
...
Mandatory skills:
- Apply `.cursor/skills/code-review-and-quality/SKILL.md` before marking any backlog item complete.
- If a spike requires architectural decisions, apply `.cursor/skills/documentation-and-adrs/SKILL.md` and record the decision.
- For frontend implementation, explicitly apply: `frontend/.cursor/skills/vercel-react-best-practices/SKILL.md`.
